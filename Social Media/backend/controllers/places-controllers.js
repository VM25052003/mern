...
const getPlacesByUserId = async(req, res, next) => {
    ...
    let identifiedPlace;
    try {
        //In model, stored as creatorId
        identifiedPlace = await Place.find({creatorId: userId})
    } catch {
        return next(new HttpError('Something went wrong. Try again later', 500))
    }
...
const createPlace = new Place({ 
    ...
    creatorId: creator, 
    image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d4/20171126_Angkor_Wat_4712_DxO.jpg/1024px-20171126_Angkor_Wat_4712_DxO.jpg',
    title, 
    description, 
    address, 
...
For this we need a relation between these two documents. Populate() would work only if connection exists. Populate() needs one additional information about the document where we want to change something. In our case, this is the 'creatorId'*/
const deletePlace = async(req, res, next) => {
    ...
    try {
        ...
        removedPlace = await Place.findById(placeId).populate('creatorId', '-password -name -email')
        //No such place exist
        if(!removedPlace){
        return next(new HttpError('No place found', 404))
         }
    ...
    try { 
            ...
            removedPlace.creatorId.places.pull(removedPlace)
            await removedPlace.creatorId.save({session: newSession})
            await newSession.commitTransaction()
        } catch {
            return next(new HttpError('Something went wrong. Try again later', 500))
        }
